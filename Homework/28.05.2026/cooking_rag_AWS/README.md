...
| Share email fails from agent | Action Lambda called `http://127.0.0.1:5001` (unreachable from AWS) | Fixed: action Lambda invokes **`cooking-rag-buddy-email` directly** using `buddy_contacts` from Flask session attributes. Run `./scripts/deploy_agent_lambda.sh` after pulling |
